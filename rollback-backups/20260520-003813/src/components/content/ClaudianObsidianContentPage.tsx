...
function CodeBlock({ code }: { code: string }) {
  ...
  const copyCode = useCallback(() => {
    navigator.clipboard.writeText(code).then(() => { setCopied(true); window.setTimeout(() => setCopied(false), 1800); });
  }, [code]);
  return (
    <div className="my-3 overflow-hidden rounded-lg border border-border bg-muted/40">
      <div className="flex items-center justify-between border-b border-border bg-muted px-4 py-2">
        <span className="font-mono text-[11px] uppercase text-foreground/70">code</span>
        <button type="button" onClick={copyCode} className="rounded-md px-2 py-1 text-xs font-medium text-foreground/70 transition-colors hover:bg-background hover:text-foreground">{copied ? '已复制' : '复制'}</button>
      </div>
...
export function ClaudianObsidianContentPage() {
  return (
    <div className="min-h-screen bg-background">
      <BreadcrumbSchema items={[{ name: 'API知识站', url: 'https://apiuspro.cn' }, { name: 'API应用', url: 'https://apiuspro.cn/app' }, { name: tutorial.name, url: 'https://apiuspro.cn/app/claudian-obsidian' }]} />
      <ArticleSchema title="Claudian Obsidian 插件安装教程" description={tutorial.desc} url="https://apiuspro.cn/app/claudian-obsidian" datePublished={ARTICLE_DATE_PUBLISHED} dateModified={ARTICLE_DATE_MODIFIED} />
      <HowToSchema name="Claudian Obsidian 插件安装教程" description={tutorial.desc} steps={tutorial.sections.flatMap((section, si) => (section.steps || []).map((step, sti) => ({ name: step.title, text: step.description || step.items?.join(';') || section.content, url: 'https://apiuspro.cn/app/claudian-obsidian#section-' + si + '-' + sti })))} totalTime="PT15M" tool={['浏览器', 'Obsidian', 'Claude Code']} />
      ...
      <main className="mx-auto max-w-[920px] border-x border-border bg-card">
        <section className="border-b border-border px-5 py-8 sm:px-10 sm:py-10 lg:px-12">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <h1 className="max-w-3xl text-[1.55rem] font-bold leading-[1.18] tracking-tight text-foreground sm:text-4xl sm:leading-tight">Claudian Obsidian 插件安装教程</h1>
          ...
          <div className="mt-8 hidden rounded-xl border border-sky-200 bg-sky-50 px-5 py-5 text-sky-900 sm:block">
          ...
          <details className="mt-6 rounded-xl border border-sky-200 bg-sky-50 text-sky-900 sm:hidden">
        ...
        <section className="hidden border-b border-border px-5 py-8 sm:block sm:px-10 lg:px-12">
          <div className="rounded-xl border border-sky-200 bg-sky-50 px-5 py-5 dark:border-sky-700/70 dark:bg-slate-950/60">
        ...
        <section className="border-b border-border px-5 py-4 sm:hidden">
          <details className="rounded-xl border border-sky-200 bg-sky-50 dark:border-sky-700/70 dark:bg-slate-950/60">
          ...
          <details>
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {tutorial.sections.map((section, index) => (
                <a key={section.title} href={'#section-' + index} className="rounded-lg border border-border bg-background/70 px-4 py-3 text-sm text-foreground/80 transition-colors hover:border-foreground/50 hover:bg-background hover:text-foreground">{index + 1}. {section.title}</a>
              ))}
            </div>
          </details>
        </section>
            ...
            <section key={section.title} id={'section-' + si} className={si < tutorial.sections.length - 1 ? 'border-b border-border pb-10' : ''}>
              ...
              <div className="space-y-7">
                {section.steps?.map((step, sti) => (
                  <article key={step.title} id={'section-' + si + '-' + sti} className="rounded-xl border border-foreground/15 bg-background/70 p-4 shadow-sm sm:p-5">
                    <h3 className="text-lg font-semibold text-foreground">{si + 1}.{sti + 1} {step.title}</h3>
                    <p className="mt-2 text-sm leading-7 text-foreground/80"><RichText text={step.description} /></p>
                    {step.code && <CodeBlock code={step.code} />}
                    {step.items && step.items.length > 0 && (
                      <ul className="mt-4 space-y-2">
                        {step.items.map((item) => <li key={item} className="flex items-start gap-2 text-sm leading-7 text-foreground/80"><span className="mt-3 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/45" /><span><RichText text={item} /></span></li>)}
                      </ul>
                    )}
                    {step.warning && <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">{'⚠ '}{step.warning}</div>}
                  </article>
                ))}
              </div>
              {section.tips && section.tips.length > 0 && (
                <div className="mt-6 rounded-lg border border-sky-200 bg-sky-50 px-5 py-4">
                  ...
                  <ul className="space-y-2">{section.tips.map((tip) => <li key={tip} className="text-sm leading-6 text-sky-900 dark:text-sky-100"><RichText text={tip} /></li>)}</ul>
                </div>
              )}
              {section.warnings && section.warnings.length > 0 && (
                <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 px-5 py-4">
                  ...
                  <ul className="space-y-2">{section.warnings.map((w) => <li key={w} className="text-sm leading-6 text-amber-900 dark:text-amber-100"><RichText text={w} /></li>)}</ul>
                </div>
              )}
            </section>
          ))}
        ...
        <section className="border-t border-border px-5 py-6 sm:px-10 lg:px-12">
          <p className="mb-3 text-sm font-semibold text-foreground/80">关键词</p>
          <div className="flex flex-wrap gap-2">
            {['Obsidian 插件', 'Claude Code', 'DeepSeek API', '笔记 + AI'].map((tag) => (
      ...
      <footer className="border-t border-border px-4 py-8 text-center text-sm text-foreground/70">
